...
function drawCitiesScatterPlot(){
	...
	xDimension = ndx.dimension(function(d){ 
		if(xScatterLabel === "Total Crimes")
				...
				return d["Total crimes per capita"];
		}),
	...
	var minX = xDimension.bottom(1)[0][xScatterLabel];
	var maxX = xDimension.top(1)[0][xScatterLabel];
...
var 	allColumns = ['City','State','Year','Population','Rapes', 'Homicides', 'Assaults', 'Robberies','Total Crimes'];
...
function drawAreaChart(){
	...
	// var avgCrimeByMonthGroup = monthsDim.group().reduceSum(function (d) {
			// return d["Population"];
	// })
			...
			.each(function(row, y) {
					d3.select(this).selectAll('td').data(cols)
							...
							.each(function(col, x) {
									...
									chart
											.transitionDuration(0)
											.width(120 + (showYAxis?20:0))
											.height(120 + (showXAxis?15:0))
											.margins({
													left: showYAxis ? 20 : 5,
													top: 5,
													right: 0,
													bottom: showXAxis ? 15 : 3
											})
